...
internal static class Program
{
    ...
    static int Main(string[] args)
    {
        try {
            ...
            return cmd switch { "add"=>Add(args,csv,corpus), "import-reviewed"=>ImportReviewed(args,csv,corpus,root), "migrate"=>Migrate(csv,corpus), "split"=>Split(csv,root), "report"=>Report(csv,Path.GetFullPath(Opt(args,"--out")??Path.Combine(root,"corpus-report.md"))), "audit"=>Audit(csv,true), _=>Usage() };
    ...
    static int ImportReviewed(string[] args,string csv,string corpus,string root)
    {
        ...
        foreach(var d in additions)AddEvidence(d.OriginalPath,d.Label,d.GroupId,SourceType(d.OriginalPath),"REVIEWED_BATCH",csv,corpus,d.ExpectedSha256);var after=Snapshot(Load(csv));WriteImportReport(root,decisionsPath,decisions,errors,before,after,true);Console.WriteLine("IMPORTACION | Aplicada=Sí | "+DescribeDelta(before,after));return 0;
    }
    ...
    static string SourceType(string path)=>Path.GetExtension(path).ToLowerInvariant() switch{".pdf"=>"PDF",".jpg" or ".jpeg" or ".png"=>"IMAGE",_=>"FILE"};
    ...
    static string Destination(string d,string h,string p){var n=string.Concat(Path.GetFileNameWithoutExtension(p).Select(x=>Path.GetInvalidFileNameChars().Contains(x)?'_':x));if(n.Length>80)n=n[..80];return Path.Combine(d,h[..12].ToLowerInvariant()+"_"+n+Path.GetExtension(p).ToLowerInvariant());}
