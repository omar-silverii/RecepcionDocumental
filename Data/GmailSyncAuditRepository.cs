...
SELECT COUNT(*) FROM dbo.GmailSyncEjecucion WHERE Id=@Id AND Estado=@State AND Estado<>N'EJECUTANDO' AND FechaFinUtc IS NOT NULL;", cn))
        ...
        public static GmailSyncAuditInfo LatestAfter(long afterId, string origin)
        {
            try
            {
                using (var cn = Connection())
                using (var cmd = new SqlCommand("SELECT TOP(1) Id,Origen,Estado,FechaInicioUtc,FechaFinUtc,MensajesEncontrados,MensajesNuevos,AdjuntosAnalizados,Facturas,Revisar,Descartados,DocumentosExistentes,Errores,UsoFallbackInicial FROM dbo.GmailSyncEjecucion WHERE Id>@AfterId AND Origen=@Origin ORDER BY Id DESC;", cn))
                {
                    cmd.Parameters.Add("@AfterId", SqlDbType.BigInt).Value = afterId;
