...
WHERE NOT EXISTS (SELECT 1 FROM dbo.DocumentoRecepcion WITH (UPDLOCK,SERIALIZABLE) WHERE GmailMensajeId=@MessageId AND GmailPartId=@PartId AND OrigenHash=@OriginHash);";
        ...
        public static IList<DocumentInfo> List(string classification)
        {
            ...
            const string sql = @"SELECT d.Id,m.FechaMensajeUtc,m.Remitente,m.Asunto,d.NombreOriginal,d.Clasificacion,d.MetodoDeteccion,d.Confianza,d.MotivoClasificacion,d.OrigenTipo,d.ResultadoRevision,d.EtiquetaRevision FROM dbo.DocumentoRecepcion d INNER JOIN dbo.GmailMensaje m ON m.Id=d.GmailMensajeId WHERE (@Classification IS NULL AND ISNULL(d.ResultadoRevision,N'')<>N'DESCARTAR') OR (@Classification=N'FACTURA' AND ISNULL(d.ResultadoRevision,d.Clasificacion)=N'FACTURA') OR (@Classification=N'REVISAR' AND d.Clasificacion=N'REVISAR' AND d.ResultadoRevision IS NULL) OR (@Classification=N'DESCARTAR' AND d.ResultadoRevision=N'DESCARTAR') ORDER BY d.FechaClasificacionUtc DESC,d.Id DESC;";
            using (var cn = new SqlConnection(ConnectionString)) using (var cmd = new SqlCommand(sql, cn))
            {
