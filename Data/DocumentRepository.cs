...
namespace RecepcionDocumental.Data
{
    ...
    public sealed class MessageDocumentInfo
    {
        ...
        public byte? Confianza { get; set; }
        public string OrigenTipo { get; set; }
        public string RutaInternaContenedor { get; set; }
        public string ResultadoRevision { get; set; }
        public string EtiquetaRevision { get; set; }
    }
...
WHERE NOT EXISTS (SELECT 1 FROM dbo.DocumentoRecepcion WITH (UPDLOCK,SERIALIZABLE) WHERE GmailMensajeId=@MessageId AND GmailPartId=@PartId AND OrigenHash=@OriginHash);";
            try
            {
                using (var cn = new SqlConnection(ConnectionString)) using (var cmd = new SqlCommand(sql, cn))
                {
                    ...
                    cn.Open(); var insertedId = cmd.ExecuteScalar();
                    if (insertedId == null || insertedId == DBNull.Value) return false;
                    InvoiceSampleService.AfterPersisted(Convert.ToInt64(insertedId));
                    return true;
                }
            }
        ...
        public static IList<DocumentInfo> List(string classification)
        {
            ...
            const string sql = @"SELECT d.Id,m.FechaMensajeUtc,m.Remitente,m.Asunto,d.NombreOriginal,d.Clasificacion,d.MetodoDeteccion,d.Confianza,d.MotivoClasificacion,d.OrigenTipo,d.ResultadoRevision,d.EtiquetaRevision,d.FechaClasificacionUtc FROM dbo.DocumentoRecepcion d INNER JOIN dbo.GmailMensaje m ON m.Id=d.GmailMensajeId WHERE (@Classification IS NULL AND ISNULL(d.ResultadoRevision,N'')<>N'DESCARTAR') OR (@Classification=N'FACTURA' AND ISNULL(d.ResultadoRevision,d.Clasificacion)=N'FACTURA') OR (@Classification=N'REVISAR' AND d.Clasificacion=N'REVISAR' AND d.ResultadoRevision IS NULL) OR (@Classification=N'DESCARTAR' AND d.ResultadoRevision=N'DESCARTAR') ORDER BY d.FechaClasificacionUtc DESC,d.Id DESC;";
            using (var cn = new SqlConnection(ConnectionString)) using (var cmd = new SqlCommand(sql, cn))
            {
        ...
        {
            const string sql = "SELECT SUM(CASE WHEN ISNULL(ResultadoRevision,Clasificacion)=N'FACTURA' THEN 1 ELSE 0 END),SUM(CASE WHEN Clasificacion=N'REVISAR' AND ResultadoRevision IS NULL THEN 1 ELSE 0 END) FROM dbo.DocumentoRecepcion;";
