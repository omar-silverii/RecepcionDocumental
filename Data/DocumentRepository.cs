...
namespace RecepcionDocumental.Data
{
    ...
    public static class DocumentRepository
    {
        ...
        {
            const string sql = @"INSERT dbo.DocumentoRecepcion (GmailMensajeId,GmailPartId,OrigenTipo,RutaInternaContenedor,OrigenHash,NombreOriginal,MimeType,TamanioBytes,RutaLocal,HashSha256,Clasificacion,MetodoDeteccion,Confianza,MotivoClasificacion,QrDetectado,FechaClasificacionUtc)
SELECT @MessageId,@PartId,@OriginType,@InternalPath,@OriginHash,@Name,@Mime,@Size,@LocalPath,@Hash,@Classification,@Method,@Confidence,@Reason,0,SYSUTCDATETIME()
